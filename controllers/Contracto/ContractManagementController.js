...
exports.getAllContract = async(req,res)=>{
    ...
    try{
        ...
        console.log('[CONTROLLER CONTRACTMANAGEMENT] Error en get AllContract',error.message);
...
exports.creatContract = async(req,res)=>{
    try{
        const{typeofcontract ,starteDate,endDate,contractNumber,state}=req.body;
        ...
        if(saveContract){
         ...
         console.log('[CONTROLLER CONTRACTMANAGEMENT] Contrato creado ',saveContract._id);
    ...
    try{
        ...
        // Verifica si el contratista existe
            if(contract){
                ...
                try{
                console.log('[CONTROLLER CONTRACTMANAGEMENT] Contratista actualizado a Inactivo');
                await contract.save();

                }catch{
                    onsole.error('[CONTROLLER CONTRACTMANAGEMENT] Error al cambiar el estado del contratista:', error.message);
                } 
            }else{
                console.warn('[CONTROLLER CONTRACTMANAGEMENT] Contratista no encontrado para actualizar el estado');
            }
        ...
        if(updateContract.state ==='Activo'){
            ...
            if(contract){
                 ...
                 try{
                    await contract.save();
                    console.log('[CONTROLLER CONTRACTMANAGEMENT] Contratista actualizado a Activo');
                 }
                 catch(error){
                    onsole.error('[CONTROLLER CONTRACTMANAGEMENT] Error al cambiar el estado del contratista:', error.message);
                 }
        ...
        console.log('[CONTROLLER CONTRACTMANAGEMENT] Contrato para actualizar ',updateContract._id);
        ...
        console.log('[CONTROLLER CONTRACTMANAGEMENT] Error al actualizar un contrato');
        return res.status(500).json({
            success:false,
