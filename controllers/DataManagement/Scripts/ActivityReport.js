...
exports.generateActivityReports = async (data) => {
  try {
    ...
    const imagesDir1 = path.join(
      __dirname,
      `${FILE}${_id}/Img/activityReport1.jpg`
    );
    const imagesDir2 = path.join(
      __dirname,
      `${FILE}${_id}/Img/activityReport2.jpg`
    );
    const imagesDir3 = path.join(
      __dirname,
      `${FILE}${_id}/Img/activityReport3.jpg`
    );
    const imagesDir4 = path.join(
      __dirname,
      `${FILE}${_id}/Img/activityReport4.jpg`
    );
    const imagesDir5 = path.join(
      __dirname,
      `${FILE}${_id}/Img/activityReport5.jpg`
    );

    // Verificar que todas las imaganes esten
    if (
      !fs.existsSync(imagesDir1) ||
      !fs.existsSync(imagesDir2) ||
      !fs.existsSync(imagesDir3) ||
      !fs.existsSync(imagesDir4) ||
      !fs.existsSync(imagesDir5)
    ) {
      ...
      messages: [
...

        {
          ...
          content: [
            ...
            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${imagenConst3}` },
            },
            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${userImagen3}` },
            },

            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${imagenConst4}` },
            },
            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${userImagen4}` },
            },

            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${imagenConst5}` },
            },
            {
              type: "image_url",
              image_url: { url: `data:image/png;base64,${userImagen5}` },
            },
          ],
        },
      ],
